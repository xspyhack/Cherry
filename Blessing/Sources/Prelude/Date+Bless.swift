...
extension Date: BlessCompatible {
}

public extension Bless where Base == Date {
    ...
    func days(with comparingDate: Date) -> Int {
        return Bless.daysOffset(between: base, and: comparingDate)
    }

    func absoluteDays(with comparingDate: Date) -> Int {
        return Bless.absoluteDaysOffset(between: base, and: comparingDate)
    }
